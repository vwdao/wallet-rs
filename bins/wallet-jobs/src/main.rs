...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // NATS consumers need direct bus access
    match NatsEventBus::connect(&cfg.nats.url).await {
        Ok(bus) => {
            let bus_arc: Arc<dyn wallet_events::EventBus> = bus;
            let tx_cons = tokio::spawn(consumer::tx_event::start(db.clone(), bus_arc.clone()));
            let dex_cons = tokio::spawn(consumer::dex_event::start(db.clone(), bus_arc));
            let _ = tokio::join!(price, token, gas, tx_cons, dex_cons);
        }
        Err(_) => {
            let _ = tokio::join!(price, token, gas);
        }
    }
    Ok(())
}
